...
public class WorkManNewsAdapter extends RecyclerView.Adapter<WorkManNewsAdapter.ViewHolder> {
    ...
    public WorkManNewsAdapter(Context context, List<WorkmanNews> items, int countNew, RecyclerViewClickListener listener) {

        if (countNew != 0) {
            int topIndex = items.size() > countNew ? countNew : items.size();
    ...
    @NonNull
    @Override
    public WorkManNewsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(
                R.layout.row_template_news, parent, false
        );

        final WorkManNewsAdapter.ViewHolder mViewHolder = new WorkManNewsAdapter.ViewHolder(view);
